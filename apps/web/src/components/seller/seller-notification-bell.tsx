...
/** Seller portal notification bell — taps resolve to seller/KYC/order routes. */
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-mn-teal px-1 text-[10px] font-bold text-white">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-80 rounded-xl border border-mn-border bg-white shadow-lg">
          ...
          <ul className="max-h-72 overflow-y-auto">
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <li
                  key={item.id}
                  className={`border-b border-mn-border/60 ${item.readAt ? '' : 'bg-mn-teal-soft/40'}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <p className="mt-1 text-[10px] font-semibold text-mn-teal">Open →</p>
                  </button>
                </li>
              ))
            )}
          ...
          <div className="border-t border-mn-border px-3 py-2">
            <Link
              href="/seller/orders"
              className="text-[11px] font-semibold text-mn-teal hover:underline"
